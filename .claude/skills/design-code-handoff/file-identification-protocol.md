...
This document describes how the skill identifies the project archive from `artifacts/docs/design/`, normalizes its location, and inventories the screens inside it. The SKILL.md Step 1 invokes this protocol; everything below applies before the inventory build.
...
The user knows what they exported and can usually answer in one sentence. A small ask up front is cheaper than a wrong "Added" or "Missing" finding the user has to override at the checkpoint.
...
- *Existing `project/` folder plus a new archive:* > *"There's already a `project/` folder from a previous run, and you just dropped in `[new file]`. Should I replace the old one with the new export, or stick with what's already there?"*
...
**First, check for an already-canonical bundle from a previous run.** If a folder named `project/` already exists at `artifacts/docs/design/project/`, use it directly — no re-identification or renaming needed. If the user dropped a new archive alongside the existing `project/` (e.g., they re-exported after editing), ask inline (see example phrasings).
...
- **Zero candidates** → ask the user to drop the project archive before continuing.

## 2. Rename to the canonical path

Once identified, rename:
- `project.zip` (if compressed) or `project/` (if already extracted).
- If it's a `.zip`, unzip it to `artifacts/docs/design/project/` — `unzip` is available in the shell.
...
Look for `README.md` at `artifacts/docs/design/project/README.md`. If the archive was unzipped with an inner project-name folder (e.g., `project/rce-5-skills-test/README.md`), look one level deeper. Once found, rename it in place to `prototype-readme.md` before reading.

If the prototype-readme is found, extract:

- **The primary file path** — the prototype-readme will name a specific screen the user had open when they triggered the handoff (e.g., *"Read `rce-5-skills-test/project/RCE/Estimates.html` in full"*). This is the **focal screen** — the prototype's intended focus.
...
> *"I can't read the prototype's screens — none of the HTML files in the project archive have content I can use. To get past this:*
>
> *1. Open `artifacts/docs/design/` on your computer.*
> *2. Delete the project archive (the `.zip` file and any `project/` folder). Leave `full-design-blueprint.md` and `HANDOFF-design-brief.md` alone.*
> *3. In Claude Design, re-export your most recent prototype as a project archive (`.zip`) — make sure you upload the full archive, not just an inner folder.*
> *4. Drop the new archive into `artifacts/docs/design/`.*
> *5. Run me again."*
