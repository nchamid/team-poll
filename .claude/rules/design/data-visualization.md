...
A table should show the columns people scan and compare, not every field. When a row carries more than fits comfortably (long text, many secondary fields, nested data), keep the table to the scannable columns and move the rest into a **side sheet** (per `disclosure-surfaces.md`), opened per row.
...
- **Sheet:** the standard side sheet — 360–480px, slides in from the right over content (`--duration-slow` `--ease-emphasis`) so the table stays visible. Header = the row's primary identifier + close (`x`); body = the full field set as label–value pairs; footer = row actions (Edit, Delete-at-bottom).
- **Keep context:** mark the open row distinctly from checkbox selection — pale-blue tint + 2px left accent, `aria-current="true"` (reserve `aria-selected` for bulk selection). Opening another row swaps the sheet content; sheets never stack. Don't widen the table past the viewport, and don't use a center modal for single-record detail — it blocks the table.
...
- Sticky header (`position: sticky; top: 0` on `<thead> <th>`, offset by top-bar height) and sticky first column on wide tables (`position: sticky; left: 0`, 4px right shadow).
...
- **Loading:** skeleton rows matching column structure. **Error:** message in the table area with retry.
- **Empty** (never had data): page-level empty state, not inline.

## Responsive
Tables: horizontal scroll with fade indicators, or stacked cards on mobile (row = card of label-value pairs) when browsed sequentially — pick one per surface. **KPI values** at 48px drop to 36px at ≤480px + `word-break: break-word` (`min-width: 0` on `.kpi`). **Horizontal bar rows** (`100px 1fr 60px`) are too greedy on phones; at ≤480px restructure so the label sits above the bar with value inline right. **KPI grids** use `repeat(auto-fit, minmax(min(100%, 220px), 1fr))` (the `min(100%, X)` floor).
