...
## Secret scanning
The CI pipeline runs Gitleaks on every push and PR (see `ci/build.yml`'s
`secret-scan` job). The `/review` skill also greps the diff for likely-secret
patterns. Any finding blocks the build until either the value is removed
and the git history is rewritten, or the value is confirmed false-positive
and allowlisted in `.gitleaks.toml` with a comment.

Pre-commit Gitleaks is optional — CI catches everything before merge.
